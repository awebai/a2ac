...
NANDA is the discovery and federation layer that sits above communication protocols. Its core artifacts:
...
NANDA explicitly does not define how agents talk to each other once they've found each other. It expects protocols like A2A, AMTP, or aWeb to handle actual communication. Think of it as DNS for agents, but with cryptographic identity and structured capability descriptions.

## What it leaves to other layers

- **Communication.** Entirely. NANDA is about finding agents, not talking to them.
- **Coordination.** Entirely. No messaging, presence, locks, or session management.

## Status

Active development. Has a working SDK and adapter. ArXiv papers published. The academic pedigree (MIT Media Lab) gives it credibility. The natural integration point with communication protocols makes it a potential unifying layer for discovery across the ecosystem.
